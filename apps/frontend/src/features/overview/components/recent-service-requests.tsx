...
async function getServiceRequests(): Promise<ServiceRequestsResponse> {
  const { getToken } = await auth();
  const token = await getToken();
  if (!token) {
    throw new Error('Unauthorized');
  }
  return dashboardApi.getServiceRequests(token);
}
...
export default async function RecentServiceRequests() {
